...
p_u_ur_los   = zeros(5,K,N_SNR);
p_u_sparse   = zeros(5,K,N_SNR);
p_u_rayleigh = zeros(5,K,N_SNR);
...
for snr_idx = 1:N_SNR
    for k = 1:1
        ...
        curvefit_sparse   = fit(psi_sparse(k,:,snr_idx)',rate_u_sparse(k,:,snr_idx)','poly2');
        curvefit_rayleigh = fit(psi_rayleigh(k,:,snr_idx)',rate_u_rayleigh(k,:,snr_idx)','poly2');
        ...
        % f_u_ur_los(k,:,snr_idx)   = p_u_ur_los(1,k,snr_idx)*psi_range_ur_los     + p_u_ur_los(2,k,snr_idx);
        f_u_ur_los(k,:,snr_idx)   = p_u_ur_los(1,k,snr_idx)*exp(p_u_ur_los(2,k,snr_idx)*psi_range_ur_los) + p_u_ur_los(3,k,snr_idx)*exp(p_u_ur_los(4,k,snr_idx)*psi_range_ur_los);
        f_u_sparse(k,:,snr_idx)   = p_u_sparse(1,k,snr_idx)*psi_range_sparse.^2     + p_u_sparse(2,k,snr_idx)*psi_range_sparse     + p_u_sparse(3,k,snr_idx);
        f_u_rayleigh(k,:,snr_idx) = p_u_rayleigh(1,k,snr_idx)*psi_range_rayleigh.^2 + p_u_rayleigh(2,k,snr_idx)*psi_range_rayleigh + p_u_rayleigh(3,k,snr_idx);
