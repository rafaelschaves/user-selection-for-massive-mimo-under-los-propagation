...
for k = 1:K
    curvefit_u = fit(psi(k,:)',rate_u(k,:)','exp2');
    curvefit_d = fit(psi(k,:)',rate_d(k,:)','exp2');

    p_u(1,k) = curvefit_u.a;
    p_u(2,k) = curvefit_u.b;
    p_u(3,k) = curvefit_u.c;
    p_u(4,k) = curvefit_u.d;

    p_d(1,k) = curvefit_d.a;
    p_d(2,k) = curvefit_d.b;
    p_d(3,k) = curvefit_d.c;
    p_d(4,k) = curvefit_d.d;

    f_u(k,:) = p_u(1,k)*exp(p_u(2,k)*psi_range) + p_u(3,k)*exp(p_u(4,k)*psi_range);       
    f_d(k,:) = p_d(1,k)*exp(p_d(2,k)*psi_range) + p_d(3,k)*exp(p_d(4,k)*psi_range);
